...
Версия: corporate-js-001-desktop-from-corporate-date-validation-strict-date-v034

ИЗМЕНЕНИЯ:
- popup date: единая маска ДД.ММ.ГГГГ и проверка реальной календарной даты без изменения дизайна, меню и остальной логики
- восстановлен desktop JS страницы «Корпоратив» вместо ошибочно подключённого mobile JS
- возвращена работа desktop-селекторов: data-corporate-page, data-corporate-gallery-open, data-review-open, data-gallery-lightbox-open
...
(function(){
  ...
  var page = document.querySelector('[data-corporate-page]');
  var stage = document.querySelector('.ip-corporate-stage');
  ...
  var galleryButton = document.querySelector('[data-corporate-gallery-open]');
  var gallery = document.querySelector('[data-corporate-gallery]');

  var scrollTopButton = document.querySelector('.ip-corporate-scrolltop');
  ...
  function ensureScrollSpacer(){
    ...
    scrollSpacer.setAttribute('data-corporate-scroll-spacer', '');
  ...
  function updateScrollTopVisibility(){
    ...
    var scaleValue = parseFloat(
      page.style.getPropertyValue('--corporate-scale')
    );
  ...
  function scrollCorporateToGallery(){
    if(isMobile()){
      return;
    }
  ...
  function setupMagneticButtons(){
    ...
    var buttons = Array.prototype.slice.call(
      document.querySelectorAll('.ip-corporate-button')
    );
  ...
  function getGalleryLightboxItems(){
    ...
    return Array.prototype.slice.call(
      gallery.querySelectorAll('.ip-corporate-gallery-item img')
    );
  }
  ...
  function updateGalleryLightboxImage(index){
    ...
    galleryLightboxImage.alt = sourceImage.alt || 'Фотография со корпоратива';
  ...
  function openCorporateGallery(){
    ...
    window.setTimeout(function(){
      scrollCorporateToGallery();
    }, 80);
  }

  function closeCorporateGallery(){
    if(!gallery || !galleryButton){
      return;
    }
  ...
  function toggleGallery(event){
    ...
    if(gallery.classList.contains('is-visible')){
      closeCorporateGallery();
      return;
    }

    openCorporateGallery();
  }
  ...
  function bindEvents(){
    ...
    if(scrollTopButton){
      scrollTopButton.addEventListener('click', scrollCorporateToTop);
    }

/*
==================================================
GRADUATION MOBILE JS

Версия: graduation-mobile-js-002-closed-page-bottom-trim-date-validation-strict-date-v034

ИЗМЕНЕНИЯ:
- popup date: единая маска ДД.ММ.ГГГГ и проверка реальной календарной даты без изменения дизайна, меню и остальной логики
- создан JS мобильной версии страницы «Выпускной» на базе approved Graduation Mobile
- селекторы, переменные, CSS custom properties и data-атрибуты переведены на graduation
- закрытая высота страницы: 2104px, раскрытая высота страницы: 5940px
- сохранены меню, popup, gallery toggle, lightbox, scrolltop и video poster logic
==================================================
*/
...
(function(){
  ...
  var page = document.querySelector('[data-graduation-mobile-page]');
  var stage = document.querySelector('.ip-graduation-mobile-content');
  ...
  var galleryButton = document.querySelector('[data-graduation-mobile-gallery-open]');
  var gallery = document.querySelector('[data-graduation-mobile-gallery]');

  var scrollTopButton = document.querySelector('.ip-graduation-mobile-scrolltop');

  var videoFrame = document.querySelector('[data-graduation-mobile-video-frame]');
  var videoPoster = document.querySelector('[data-graduation-mobile-video-poster]');

  var lightbox = document.querySelector('[data-graduation-mobile-lightbox]');
  var lightboxImage = document.querySelector('[data-graduation-mobile-lightbox-image]');
  var lightboxCounter = document.querySelector('[data-graduation-mobile-lightbox-counter]');
  var lightboxCloseButton = document.querySelector('.ip-graduation-mobile-lightbox-close');
  var lightboxCloseTriggers = Array.prototype.slice.call(
    document.querySelectorAll('[data-graduation-mobile-lightbox-close]')
  );

  var galleryLightboxItems = Array.prototype.slice.call(
    document.querySelectorAll('[data-graduation-mobile-lightbox-open]')
  );
  ...
  function updateScale(){
    ...
    if(isMobile()){
      ...
      document.documentElement.style.setProperty(
        '--graduation-mobile-scale',
        scale.toFixed(5)
      );

      document.documentElement.style.setProperty(
        '--graduation-mobile-page-height',
        scaledHeight + 'px'
      );
  ...
  function closeGallery(){
    ...
    window.setTimeout(function(){
      ...
      window.requestAnimationFrame(function(){
        scrollToElement(document.querySelector('.ip-graduation-mobile-about'), 36);
      });
    }, 520);
  }

...
Версия: graduation-js-003-approved-menu-source-date-validation-strict-date-v034
...
- сохранены меню, popup, галерея, lightbox, отзывы, scroll-top и масштабирование
==================================================
*/
...
(function(){
  ...
  var page = document.querySelector('[data-graduation-page]');
  var stage = document.querySelector('.ip-graduation-stage');
  ...
  var galleryButton = document.querySelector('[data-graduation-gallery-open]');
  var gallery = document.querySelector('[data-graduation-gallery]');

  var scrollTopButton = document.querySelector('.ip-graduation-scrolltop');
  ...
  function ensureScrollSpacer(){
    ...
    scrollSpacer.setAttribute('data-graduation-scroll-spacer', '');
  ...
  function updateScrollTopVisibility(){
    ...
    var scaleValue = parseFloat(
      page.style.getPropertyValue('--graduation-scale')
    );
  ...
  function scrollGraduationToGallery(){
    if(isMobile()){
      return;
    }
  ...
  function scrollGraduationToTop(){
    page.scrollTo({
      top:0,
      behavior:'smooth'
    });
  ...
  function setupMagneticButtons(){
    ...
    var buttons = Array.prototype.slice.call(
      document.querySelectorAll('.ip-graduation-button')
    );
  ...
  function getGalleryLightboxItems(){
    ...
    return Array.prototype.slice.call(
      gallery.querySelectorAll('.ip-graduation-gallery-item img')
    );
  }
  ...
  function updateGalleryLightboxImage(index){
    ...
    galleryLightboxImage.alt = sourceImage.alt || 'Фотография с выпускного';
  ...
  function openGraduationGallery(){
    ...
    window.setTimeout(function(){
      scrollGraduationToGallery();
    }, 80);
  }

  function closeGraduationGallery(){
    if(!gallery || !galleryButton){
      return;
    }
  ...
  function toggleGallery(event){
    ...
    if(gallery.classList.contains('is-visible')){
      closeGraduationGallery();
      return;
    }

    openGraduationGallery();
  }
  ...
  function bindEvents(){
    ...
    if(scrollTopButton){
      scrollTopButton.addEventListener('click', scrollGraduationToTop);
    }
